...
import { Users, Plus, Search, BookOpen, Edit2, ToggleLeft, ToggleRight } from 'lucide-react';
...
export default function BuyersPage() {
  ...
  const fetchBuyers = () => {
    fetch(`/api/buyers?search=${search}`)
      .then(res => res.json())
      .then(data => setBuyers(data.buyers || []))
      .finally(() => setLoading(false));
  };

  useEffect(() => { fetchBuyers(); }, [search]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const url = editId ? `/api/buyers/${editId}` : '/api/buyers';
  ...
  const handleEdit = (b: any) => {
    setEditId(b._id);
    setForm({ name: b.name, phone: b.phone, address: b.address || '', email: b.email || '', cnic: b.cnic || '', openingBalance: b.openingBalance || 0, notes: b.notes || '' });
    setShowForm(true);
  };

  const toggleActive = async (b: any) => {
    await fetch(`/api/buyers/${b._id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ isActive: !b.isActive }) });
    fetchBuyers();
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage your buyers and their accounts</p>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-100 p-6 mb-6">
          <h3 className="text-sm font-semibold text-gray-900 mb-4">{editId ? 'Edit Buyer' : 'New Buyer'}</h3>
      ...
      <div className="bg-white rounded-xl border border-gray-100">
        ...
        {buyers.length === 0 ? (
          <div className="p-12 text-center"><Users className="w-10 h-10 text-gray-300 mx-auto mb-3" /><p className="text-sm text-gray-500">No buyers found</p></div>
        ) : (
              ...
              <tbody className="divide-y divide-gray-50">
                {buyers.map(b => (
                  <tr key={b._id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-5 py-3 text-sm text-gray-600">{b.address || '-'}</td>
                    <td className="px-5 py-3 text-sm text-gray-600">{formatCurrency(b.openingBalance || 0)}</td>
                    <td className="px-5 py-3"><span className={`text-xs px-2 py-1 rounded-full ${b.isActive ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{b.isActive ? 'Active' : 'Inactive'}</span></td>
                    <td className="px-5 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link href={`/dashboard/buyers/${b._id}`} title="View ledger" className="text-gray-400 hover:text-primary-600"><BookOpen className="w-4 h-4" /></Link>
                        <button onClick={() => handleEdit(b)} className="text-gray-400 hover:text-primary-600"><Edit2 className="w-4 h-4" /></button>
                        <button onClick={() => toggleActive(b)} className="text-gray-400 hover:text-gray-600">
                          {b.isActive ? <ToggleRight className="w-5 h-5 text-green-500" /> : <ToggleLeft className="w-5 h-5" />}
                        </button>
                      </div>
                    </td>
                  </tr>
